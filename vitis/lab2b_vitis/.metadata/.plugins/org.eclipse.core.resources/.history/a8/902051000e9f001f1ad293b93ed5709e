...
void draw(void){
		...
		setColor(255, 0, 0);
		float percent = (float) volume / 63;
		int xVal = 200 * percent;
		fillRect(20, 50, 20+xVal, 70);
		char text[5] = {'M','O','D','E',' '};
		setColor(0, 0, 100);
		fillRect(56, 75, 184, 101);
		setColor(0, 0, 255);
		for(int i = 0; i < 5; i ++){
			printChar(text[i], cfont.x_size * i + 80, 80);
		}
		printChar(button, cfont.x_size * 5 + 80, 80);
}
...
void update_volume(int new_vol){
	int vol_diff = new_vol - volume;
	int left, right;
	float p_o = (float) volume / 63;
	float p_n = (float) new_vol / 63;
	if(vol_diff > 0){
		setColor(255, 0 ,0);
		left = 20 + p_o * 200;
		right = 20 + p_n *200;
